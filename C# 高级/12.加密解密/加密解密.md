项目中，有时候会对重要数据进行加密和解密，比如密码。加密算法分为两类：可逆和不可逆。可逆就是加密后能解密回原文，不可逆就是加密后无法还原为原文。
...
> **注意**：MD5 目前已被证明存在碰撞攻击，已被认为不再安全。在安全场景（如密码存储、数字签名）中，建议使用 SHA-256 或更高强度的算法，或使用 bcrypt、Argon2 等专门的密码哈希算法。
