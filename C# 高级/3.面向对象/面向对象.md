...
> **补充**：从 C# 8.0 开始，接口支持默认实现（default interface methods），即可以在接口中为方法提供默认的实现体。这样在接口中新增方法时，已有的实现类就不必全部修改。但这仍然是相对少用的特性，核心原则没有变——接口主要定义"能做什么"的契约，而不是"怎么做"。
