...
// ThreadWithReturn 会开启一个新线程去执行 func，本身立即返回，不阻塞当前线程
...
// funcThread.Invoke() 内部调用了 thread.Join()，会阻塞直到子线程完成，拿到返回值
...
若是想捕获多线程异常，可以使用多线程专用异常类：AggregateException ，并循环获得每个异常的信息。

常规做法是将try catch放在线程内部，也就是Task.Run()里。

## 线程取消

多线程并发任务，某个线程失败后，希望把后面的线程都停下来，CancellationTokenSource类可以实现。
...
线程内遇到异常时，需要设置cts.Cancel();后续的线程就会取消，取消的操作会有延迟，无法保证具体到某个线程取消。
...
![img](https://cdn.nlark.com/yuque/0/2026/png/42432244/1768919295139-46794ee8-3cdd-485b-a7e9-213f2735c351.png)

可以看到这五个输出的i都是5，为什么会这样？因为线程是非阻塞的，延迟启动的，当线程执行的时候，i已经是5了，而k是闭包的，每个循环是独立的。