...
这里显示PriceChangedHandler只能出现在 += 或 -= 的左边，使用PriceChangedHandler = 方法名 语句是错误的。这就是事件的作用，事件相当于是对一部分委托进行封装，使用事件时会运行封装内的代码，在这基础上再运行 += 右边的代码，委托不一样，委托可以通过=直接赋值，缺少了封装性。这也是事件相比委托的优点。