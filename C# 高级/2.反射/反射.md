...
我们只需要将DB.Oracle里debug里的.dll文件复制到Reflection的debug里，修改配置项，运行.exe文件：
...
因此，反射性能问题确实存在，但这是循环10000000次的结果，实际项目中不会循环这么多，所以无需因为性能问题去避开反射。并且比较reflectionTime1和reflectionTime2，可以看到性能大部分花在了加载dll和获取类型上面，项目在运行时，可以先进行动态加载，预处理，这样可以最大幅度减少反射的性能问题。