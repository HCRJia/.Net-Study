设计模式原则，就是在面向对象语言开发过程中，推荐的一些指导性原则，一些项目构成的建议，不需要强制遵守。虽然原则不是编译器强制的，但在团队协作和架构设计中，它们都是要尽力去遵守的，否则代码很容易变得难以维护。
...
子类应该能够完全替代父类，而不改变程序的正确性。如果子类需要**改变**父类的行为，或者**缺少**父类的某些核心能力，那么这样的继承关系可能就不合理，应该考虑重构。子类可以有自己的属性和行为，父类实现的东西，子类就不要去替换，这就是里氏替换原则。

# 依赖倒置原则

高层模块不应该依赖低层模块，二者都应该于抽象，依赖抽象，而不是依赖细节。如果高低层直接依赖的话，那么改动低层也会影响高层的代码。会影响项目。
...
这种写法就是高低层的一种依赖，如果要再加一个手机对象，供Student使用呢，那高低层都要写，并且高层的代码会受低层的影响，就要求先开发低层代码再开发高层代码，如果工作中高低层是交给两个人开发的，那就会有影响。
...
那么可以改成依赖倒置原则，增加一个抽象类，使高低层都访问这个抽象类。
...
这样再加手机类并继承AbstractPhone类的话就可以直接调用Play了。

# 接口隔离原则

接口隔离是客户端不应该依赖它不需要的接口；一个类对另一个类的依赖应该建立在最小的接口上。简单来讲继承一个接口能够实现该接口的所有方法。
...
iPhone类继承这个接口，因为iPhone可以用到IExtend的所有方法，这个没问题。
...
但我要换一种设备，比如游戏机，只有上面的Online和Game功能，那么怎么做？
	要实现接口隔离原则，那就拆分接口：
...
然后iPhone类就要再继承IExtendGame接口，否则会报错。

```csharp
public class PS4 : IExtendGame
{
    public void Online()
    {
        Console.WriteLine("User {0} Online", this.GetType().Name);
...
但如果再来一个不能联网的游戏机呢，又要把Game再拆开来，那就变成一个方法一个接口了，那也不行。

所以接口的定义不能太大，不然会有很多用不到的，也不要太细，不太可行，需要根据业务需求去调整。

# 迪米特原则

迪米特原则又称为 最少知道原则，它表示一个对象应该对其它对象保持最少的了解。通俗来说就是，只与直接的朋友通信。
...
开闭原则的意思是对扩展开发，对修改关闭，也就是尽量不要去修改已经写好的类，用继承，扩展方法等对类进行扩展的开发。